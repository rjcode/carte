...
["SELECT artist.id AS artist_id, artist.name AS artist_name, album.id AS album_id, album.title AS album_title, track.id AS track_id, 
...
Carte is great for generating massive nested data structures. That is only half of the problem. How to we update them in a way that keeps the data consistent and also allows us to easily save the changes that we have made?
...
Finally there is the <code>find-in</code> function which allows you to query a nested data structure.
