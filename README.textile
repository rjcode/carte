...
If you would like to experiment with carte then the best way would be to follow these instructions which assume that you have MySQL installed and that you are using leiningen. 
...
This will run a bunch of unit tests and will create some tables in the test database which we created in the previous step. Start up a REPL and execute the following expressions.
...
The <code>carte.test-core</code> namespace set up our database connection, created a data model and created some helper functions for us. <code>default-test-data</code> loaded sample data that we may experiment with. We are now ready to run some queries.
...
Note that metadata is used to store the name of the table that each record comes from as well as the original value of each record. This is used to determine if a record is dirty and needs to be updated.
...
["SELECT album.id as album_id, album.title as album_title 
   FROM album WHERE album.title like ?" "Mag%"]
({:id 4 :title "Magic Potion"})
...
["SELECT album.id as album_id, album.title as album_title 
   FROM album 
   WHERE (album.title like ? AND album.id = ?) 
   OR (album.title = ?)" "Mag%" 4 "Let's Dance"]
({:id 4 :title "Magic Potion"} 
 {:id 6 :title "Let's Dance"})
...
["SELECT artist.id as artist_id, artist.name as artist_name 
   FROM artist WHERE artist.name = ?" "David Bowie"] 
...
["SELECT * FROM album_artist 
   WHERE album_artist.artist_id = ? AND album_artist.album_id = ?" 4 6]
6
...
In order for this to work, we need a database connection and a data model definition. We skipped this part in the example above. It was set up for us in the <code>carte.test-core</code> namespace. Here we see the connection that was used above.
...
Next, create a data model using the <code>model</code> macro. This example only shows creating many-to-many associations but there is also currently support for one-to-many. In the future there will be support for belongs-to and one-to-one associations.
...
<ul>
	<li>Figure out how to best support arbitrary SQL with &lt;, &gt;, &lt;=, etc.</li>
